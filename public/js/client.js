// CONSTANTS
const gameHeight = 720; // game height (in pixels)
const gameWidth = 3000; // game total (not shown) width (in pixels)
const gameFloor = 140; // game floor height (in pixels)
const canvas = document.getElementById("gameCanvas"); // canvas variable
const ctx = canvas.getContext("2d"); // canvas context

canvas.width = 1080; // setting canvas width (in pixels).
canvas.height = gameHeight; // setting canvas height (in pixels)

let cameraSpeed = 0; // cameraSpeed changes the cameraX variable
let cameraX = 0; // cameraX is used to offset everything along the x axis

// InputHandler: Holds an array of the currently pressed keys.
class InputHandler {
    constructor() {
        this.pressedKeys = []; // array of currently pressed keys
        
        // add a listener to add the left or right arrow key to the pressedKeys array when pressed
        ...
        // add a listener to remove the left or right arrow key from the pressedKeys array when released
...
/* 
* Acts as a blueprint to build a soldier. Has all the base stats of the soldier such as
    * name,
    * strength,vitality, agility, luck, range, and
    * spriteName.
* Also has a resourceCost which determines how much it costs to spawn a soldier.
* Also has a level, xp, and xpMax for soldier progression. This improves the stats of all
  future soldiers made with this soldierType.
* Also has an xpReward which determines how much xp this soldier gives when it's killed.
*/
class SoldierType {
    ...
    // Check if the xp is higher than xpMax and if it is, increase the soldiertype's
    // level, set its xp to 0, and increase its xpMax, stats, xpReward, and resourceCost.
    update() {
...
/*
* Represents a soldier that's based on a soldierType and belongs to a team.
*/
class Soldier {
    constructor(team, soldierType) {
        // Constructor assignments
        this.team = team;
        this.soldierType = soldierType;

        // Castle assignment
        this.castle = this.team.castle;
        ...
        // Sprite source width and height (in pixels)
        this.sourceWidth = 32;
        this.sourceHeight = 32;

        // Y position (sets them to the floor)
        ...
        // Targeting/attacking/moving variables
        this.moveable = true;
        this.target = null;
        this.timeOnAttack = 0;
        ...
        // Set the x position (in pixels) of the soldier based on if it's on the client player's team or not
    ...
    // Checks all targets that are not this soldier and on the other side to see 
    ...
    // Sets this soldier's target to the given object if it is valid and in colision.
...
class Background {
    ...
    // Increases the camera speed depedning on if the left or right arrow are pressed
    // and then adjust the cameraX accordingly.
    update(input) {
...
// Stationary unit that is targetable and has a team and healthbar.
class Building {
    image;
    x;
    y;
...
// A special building that has a number of building plots that allow for new buildings
// to be built and lays them out in the right direction depending on which team.
class Castle extends Building {
    ...
    // Update each building in each plot that is not empty.
    update() {
        super.update();
        this.buildingPlots.forEach(buildingPlot => {
            buildingPlot.update();
...
// A type of building that can be destroyed (ie all buildings other than castle)
...
// Increases this team's max housing while alive.
...
// Holds a build 'button' that is shown to the player that can be used to build new buildings
// or it holds a building if one has been built.
class BuildingPlot {
    constructor(team, x) {
        this.team = team;
...
// Using a unique id, sets itself to one of 6 buildings and sets the dialogue buttons
// if clicked on and builds the corresponding building if the dialogue is confirmed.
...
// Floating number that displays how much damage a soldier did. 
class DamageNumber {
...
// Contains the buildings, soldiers, username, and more of a team. IsPlayer is used
// to determine which side of the map to show the teams assets.
class Team {
